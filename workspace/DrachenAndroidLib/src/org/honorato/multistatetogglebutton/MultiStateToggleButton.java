...
import com.vsis.drachen.android.R;

public class MultiStateToggleButton extends ToggleButton {

	private static final String TAG = MultiStateToggleButton.class
			.getSimpleName();
	...
	public MultiStateToggleButton(Context context, AttributeSet attrs) {
		...
		int[] set = { android.R.attr.entries };
	...
	/**
	 * If multiple choice is enabled, the user can select multiple values
	 * simultaneously.
	 * 
	 * @param enable
	 */
	...
	/**
	 * Set multiple buttons with the specified texts and default initial values.
	 * Initial states are allowed, but both arrays must be of the same size.
	 * 
	 * @param texts
	 *            An array of CharSequences for the buttons
	 * @param selected
			...
			if (i == 0) {
				...
				if (texts.length == 1) {
					b = (Button) inflater.inflate(
							R.layout.view_single_toggle_button, mainLayout,
							false);
				} else {
					b = (Button) inflater
							.inflate(R.layout.view_left_toggle_button,
									mainLayout, false);
				}
